...
data3_df = pd.read_csv('Data3.csv')
...
combined_df = pd.concat([data1_df, data2_df, data3_df], axis=1)
...
# Example: Histogram of 'number of reviews'
plt.figure(figsize=(8, 6))
sns.histplot(combined_df['number of reviews'], bins=20, kde=True)
...
# Example: Countplot of 'Free wifi'
plt.figure(figsize=(8, 6))
sns.countplot(data=combined_df, x='Free wifi')
plt.xlabel('Free Wifi')
plt.ylabel('Count')
plt.title('Count of Hotels with Free Wifi')
plt.show()
...
# Data Distribution Example (you can create more plots for other columns)
plt.figure(figsize=(8, 6))
sns.histplot(combined_df['number of reviews'], bins=20, kde=True)
plt.xlabel('Number of Reviews')
plt.ylabel('Frequency')
...
# Categorical Data Analysis Example (you can create more plots for other columns)
plt.figure(figsize=(8, 6))
sns.countplot(data=combined_df, x='Free wifi')
plt.xlabel('Free Wifi')
plt.ylabel('Count')
...
# Custom Analysis (add your custom analysis here based on project requirements)

# Write detailed observations and insights based on your analysis
...
non_numeric_columns = ['Hotel name', 'cities', 'location', 'Free wifi', 'Family rooms', 'Non Smoking Rooms',
                       'Restaurant', 'Bar', 'Heating', 'Lift', 'Breakfast Cuisine1', 'Breakfast Cuisine2',
                       'Breakfast Cuisine3', 'Room Type', 'Single Bed', 'Double bed']

# Exclude non-numeric columns from correlation analysis
numeric_columns = combined_df.drop(columns=non_numeric_columns)
